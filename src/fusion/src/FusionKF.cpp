...
void FusionKF::Predict(Eigen::Ref<Eigen::Matrix2Xd> m_fusionCones, Eigen::Ref<Eigen::MatrixX2d> m_fusionConesCov, uint8_t size)
{
    ...
    {
        m_fusionCones.col(i) = m_A * m_fusionCones.col(i);
        m_fusionConesCov.block<2,2>(2*i,0) = m_A * m_fusionConesCov.block<2,2>(2*i,0) * m_A.transpose() + m_Q;
    }
}
                    ...
                    const Eigen::Ref<const Eigen::Vector2d> &measurement, const Eigen::Ref<const Eigen::Matrix2d> measModel)
{
    ...
    m_fusionCones.col(idx) += K * (measurement - m_H * m_fusionCones.col(idx));
